const API_KEY = 'mf62gzMJkAtGMLmpxLBUd6YP'; //Greener Api
...
const fs = document.querySelector(".front-side");
...
function processFile(file) {
    ...
    reader.onload = (e) => {
        ...
        folder.style.display = 'none'; //for hide folder uplo
...
async function removeBackground() {
    ...
    try {
        ...
        let originalName = originalFile ? originalFile.name.replace(/\.[^/.]+$/, "") : 'processed';
        document.querySelector('#PicName').innerText = `BG_Dragger_${originalName}.png`; 
        document.querySelector('.RDone').style.display = "block"; 
        ...
        ${error}, Check Your Internet Connection Or wait for new update.`;
...
function downloadResult() {
    ...
    link.download = `BG_Dragger_${originalName}.png`;
